...
export default function MetricsGrid({ theme, patientData }) {
  const metrics = [
    ...
    },
    {
      ...
      iconColor: "text-green-500",
      trend:
        patientData?.completed > 0 ? "Successfully done" : "No completed yet",
      trendColor: "text-green-500",
      trendBold: true,
    },
    {
      ...
      trend:
        patientData?.cancelled > 0 ? "Cancelled bookings" : "None cancelled",
      ...
      {metrics.map((metric) => {
        ...
        return (
          ...
          >
            ...
            <h4
              className={`text-3xl font-bold mt-3 ${theme === "dark" ? "text-white" : "text-slate-900"}`}
            >
              {patientData === null ? (
            ...
            <div
              className={`flex items-center gap-1 mt-2 text-sm ${metric.trendColor} ${metric.trendBold ? "font-bold" : "font-medium"}`}
            >
