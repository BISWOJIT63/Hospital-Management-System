...
export const SidebarContent = ({ active, setActive, time, onLogout, onClose }) => (
    <div style={{ display: "flex", flexDirection: "column", height: "100%" }}>
        <div style={{ padding: "18px 16px", borderBottom: "1px solid var(--border)", display: "flex", alignItems: "center", justifyContent: "space-between" }}>
            <div style={{ display: "flex", alignItems: "center", gap: 10 }}>
                <div style={{ width: 32, height: 32, background: "linear-gradient(135deg,#00f5d4,#7b2ff7)", borderRadius: 7, display: "flex", alignItems: "center", justifyContent: "center", color: "#fff" }}><Activity size={18} /></div>
                <div>
                    <div className="orbitron" style={{ fontSize: 11, fontWeight: 800, color: "#00f5d4", letterSpacing: 1 }}>HMS NEXUS</div>
        ...
        <nav style={{ flex: 1, padding: "12px 7px", overflowY: "auto" }}>
            ...
            {navSections.map(s => (
                <div key={s.id} className={`nav-item ${active === s.id ? "active" : ""}`} onClick={() => { setActive(s.id); onClose && onClose(); }}
                    ...
                    {s.badge && <span style={{ background: "#f72585", color: "#fff", borderRadius: 10, padding: "1px 6px", fontSize: 9, fontFamily: "Share Tech Mono" }} className="notif-dot">{s.badge}</span>}
                </div>
            ))}
        </nav>
